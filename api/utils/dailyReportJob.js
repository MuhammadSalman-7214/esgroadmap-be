...
export const runDailyReportJob = async () => {
    ...
    for (const search of searches) {
        try {
            const uploadDateFilter = search.lastChecked !== null
            ...
            console.log(`Found ${results.length} results for keyword "${search.keyword}" in target sentence "${search.tableName}"`);
            if (results.length > 0) {
                const pdfPath = path.join(reportsDir, `report-${search.userId}-${Date.now()}.pdf`);
                await new Promise((resolve, reject) => {
                    ...
                    doc
                        .font("Helvetica-Bold")
                        .fontSize(16)
                        .text(`Search Report for keyword "${search.keyword}" in target sentence "${search.tableName}"`, { align: "center" });
                    doc.moveDown(1.5);
                    const headers = [
                        "ID",
                        "Company",
                    ...
                    results.forEach((item) => {
                        ...
                        const rowHeight = Math.max(...cellHeights.map((h) => h + padding * 2), 20);
                ...
                await sendEmail(search.user.email, `Daily Report - Keyword: ${search.keyword}`, `<p>Hello, please find today's report for your saved search: <strong>${search.keyword}</strong>.</p>`, pdfPath);
